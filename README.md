...
O arquivo `gerenciador_chamados.py` implementa um protótipo de gestão de chamados usando uma lista de dicionários. Cada chamado possui `id`, `titulo`, `prioridade`, `situacao` e `categoria`.

O programa demonstra as operações solicitadas na atividade:

| Operação | Implementação |
| --- | --- |
| Listagem geral | Exibe todos os chamados com um laço `for`. |
| Filtro por situação | Mostra chamados de uma situação existente e informa quando não há resultados. |
| Atualização por ID | Altera a situação de um chamado e trata IDs inexistentes. |
| Categorias únicas | Usa um conjunto (`set`) para eliminar categorias repetidas. |

Para executar:

```bash
py gerenciador_chamados.py
```

No ambiente em que o comando padrão é `python3`, utilize:
